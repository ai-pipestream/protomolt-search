...
## Whole segments are skipped from their summaries

Each sealed segment records, per integer and double column, its least and
greatest stored value and how many documents carry one. Before a shard resolves
a filter over its slots, it compares the filter with those ranges and drops
every segment no document of which could pass: a range predicate outside the
segment's range, or on a column the segment does not hold. A dropped segment is
not opened by the vector scan, the postings walk, a facet count, an aggregation,
or a browse. Only sound rules apply: `NOT`, an `OR` with an unbounded branch,
and facet, map, geo, and string predicates never drop a segment on their own.
The answer is identical with pruning off (`--segment-pruning=false`); the
profile reports `segments_total` and `segments_skipped`. Rows sit in ingest
order unless a compaction ordered them, so the skips pay when the layout groups
values; see the partitioned layout in chapter 10.
