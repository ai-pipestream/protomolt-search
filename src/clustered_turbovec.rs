...
/// What the clustered backend reports for the dense execution policy
/// and the quality profile: the live identity the coordinator binds to.
/// Available in every build; the client that fills it needs `net`.
...
#[cfg(feature = "net")]
mod imp {
    use super::ClusteredQualityIdentity;
    use std::pin::Pin;
    use std::sync::Arc;
    ...
    use turbovec_grpc::proto::{
        ...
        CollectionSearchRequest, CollectionSearchResponse, FloorUpdate, LabelBitmap,
        ListNodesRequest, ListNodesResponse, StartCollectionCandidates, StopStreamSearch,
    };
                ...
                Some(collection_candidate_response::Payload::Batch(batch)) => {
                    ...
                    let candidates = batch
                        ...
                        .map(|record| ClusteredCandidate {
                            label: u64::from_le_bytes(
                                record[..8].try_into().expect("8-byte label"),
                            ),
                            score: f32::from_le_bytes(
                                record[8..12].try_into().expect("4-byte score"),
                            ),
                        })
                        .collect();
