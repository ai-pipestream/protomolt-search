...
pub(crate) fn evaluate_boolean_membership(
    ...
    let n = docs.max(guard.index.as_ref().map_or(0, VectorIndex::len));
    ...
    for (index, leaf) in req.leaves.iter().enumerate() {
        ...
        let mut membership = match leaf.leaf.as_ref() {
            ...
            Some(L::Dense(_)) => Membership::Bits(Bits::prefix(
                n,
        ...
        if let Membership::Bits(bits) = &mut membership {
    ...
    fn run_evaluate_boolean(
            ...
            _ => {
                ...
                for &leaf_index in &scoring {
                    match req.leaves[leaf_index].leaf.as_ref() {
                        ...
                        Some(L::Dense(dense)) => {
                            let in_leaf: Vec<u32> = member_slots
                                .iter()
                                .copied()
                                .filter(|slot| {
                                    evaluated.leaves[leaf_index].membership.test(*slot as usize)
                                })
                                .collect();
        ...
        for &leaf_index in &scoring {
            let scores = match req.leaves[leaf_index].leaf.as_ref() {
                ...
                Some(L::Dense(dense)) => {
                    let in_leaf: Vec<u32> = candidate_slots
                        .iter()
                        .copied()
                        .filter(|slot| evaluated.leaves[leaf_index].membership.test(*slot as usize))
                        .collect();
                    let mut spread = vec![None; candidate_slots.len()];
