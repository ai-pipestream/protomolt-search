...
it. A filter leaf the tree reaches only under MUST, beside a lexical
sibling, resolves its column reads over the intersection of those
...
is never consulted. SHOULD and MUST_NOT filters and filters without a
lexical MUST sibling still resolve over the whole shard. Leaves resolve
cheapest first — lexical clauses, then filters, then nested groups, then
dense clauses — and once a group's MUST intersection is empty on a
MUST-only path from the root, the group and the root admit no row, so the
remaining leaves resolve as the empty set (a filter still validates and
reports its handshake; a dense clause's bitmap is never built).
