...
- **Foundation branch: bounded source-owned compaction.** The owner rebuilds
  ...
  journal. The embedded Rust operation owns private files through awaiter
  cancellation. WAL compaction remains separate. See
...
- **Foundation branch: journaled source-owned cutover.** The local owner can
  ...
  Bounded rebuilding and the embedded operation are covered above; the
...
- **Foundation branch: persistent generation declarations.** Source publication
  ...
  verifies final-segment reclamation. Source-owned rebuilding, journal and
  cutover are covered by newer entries above. See
  [source-index maintenance](docs/source-index-maintenance.md).
- **Foundation branch: source rewrite preservation check.** A bounded row-digest
  ...
  now has its builder, journal, cutover and generation declarations covered
  by the newer entries above.
...
- **2026-09-06 — Field-authorization integration with current main.** The
  ...
  and CI prerequisites, plus the replica-bootstrap test fix through `5ded652`.
  See the current integration validation
...
- **Branch checkpoint 2026-09-07: missing catalog recovery refuses.**
  Persistent document-catalog reopen no longer creates fresh version/retry
  history when the file is missing. Embedded startup fails before shard storage
  is opened; the mobile ABI reports `NOT_FOUND`. Explicit creation remains a
  separate operation. [Document writes](docs/document-writes.md).
...
- **Branch checkpoint 2026-09-07: the rewrite proof certifies the dense
  ...
  under umask 0. Tests: `segments::rewrite_proof`, `vector::tests`.
  [Source-index maintenance](docs/source-index-maintenance.md).
