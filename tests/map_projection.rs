...
fn maps_refuse_unimplemented_numeric_or_structural_projections() {
    for (value_type, kind) in [
        ...
        (Type::Int64, pb::MappedKind::Int64),
...
fn chunk_maps_keep_each_element_separate() {
    ...
    let extractor =
        Extractor::with_definition(&schema, "map_projection.Parent", "", Some(&policy)).unwrap();
    ...
    let chunks = ["first", "second"]
        ...
        .map(|text| {
            ...
            chunk.set_field_by_name(
                "attributes",
                Value::Map(
                    [(MapKey::String("".into()), Value::String(text.into()))]
                        .into_iter()
                        .collect(),
                ),
            );
    ...
    for (row, text) in rows.iter().zip(["first", "second"]) {
        assert_eq!(
