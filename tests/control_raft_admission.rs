...
//! `control_raft_snapshots` convention plus `fault-injection`: the pause
//! hooks `arm_grant_gate` and `arm_precommit_pause` are fault-injection,
//! and leader isolation needs `tls`). All timing derives from
