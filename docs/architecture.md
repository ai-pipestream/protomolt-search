...
search, variant search, aggregation, autocomplete and did-you-mean, and
ingest administration, over TLS with a bearer principal once the coordinator
is configured for it (`docs/security.md`). Node to cluster control is the
ClusterControlService: registration, leases, shard reports, and placement
actions such as replica bootstrap and the online split
(`docs/cluster-control.md`). Coordinator to node is the
