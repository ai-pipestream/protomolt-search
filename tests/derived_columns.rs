...
fn document(i: usize) -> AddDocumentsRequest {
    AddDocumentsRequest {
        ...
        map_integers: vec![pipestream_search::pb::MapIntegerEntry {
            field: "signed_map".into(),
            key: "".into(),
            value: i64::MIN + i as i64,
        }],
        map_unsigned_integers: vec![pipestream_search::pb::MapUnsignedIntegerEntry {
...
fn config(index_path: Option<PathBuf>, derived: Option<Arc<Declaration>>) -> NodeConfig {
    NodeConfig {
        ...
        map_integer_fields: vec!["signed_map".into(), "absent_signed".into()],
        map_unsigned_integer_fields: vec!["unsigned_map".into(), "absent_unsigned".into()],
...
async fn projected(
    ...
    let response = root
        .bm25_search(Request::new(Bm25SearchRequest {
            ...
            projections: vec![
                ...
                projection("signed_map", "signed_map['']"),
                projection("unsigned_map", "unsigned_map['']"),
            ],
            ..Default::default()
        }))
        .await?
    ...
    for hit in response.hits {
        ...
        let ordinal = (0..N).find(|i| decided(*i) == int(0)).unwrap();
        assert_eq!(int(4), i64::MIN + ordinal as i64);
        assert_eq!(uint(5), u64::MAX - ordinal as u64);
...
async fn derived_columns_are_computed_stored_pinned_and_queryable() {
    ...
    // Every sealed segment carries the kind-17 entry, and the log's
    ...
    assert_eq!(columns.map_integers, vec!["signed_map", "absent_signed"]);
    assert_eq!(
        columns.map_unsigned_integers,
        vec!["unsigned_map", "absent_unsigned"]
    );
...
#[tokio::test]
async fn an_undeclared_fingerprint_is_refused_without_allocating_a_row() {
    for layout in [Layout::SingleImage, Layout::Segments] {
        let (addr, server) = start_empty_node(NodeConfig {
            layout,
            analysis_addr: Some(NATIVE_ANALYSIS_BACKEND.to_string()),
            ..Default::default()
        })
        ...
        let error = client
            .add_documents(tokio_stream::iter([AddDocumentsRequest {
                text: "opinion".into(),
                analysis: Some(body_spec()),
                derived_fingerprint: declaration().fingerprint().to_string(),
                ..Default::default()
            }]))
            .await
            .unwrap_err();
        assert!(
            error.message().contains("declares no derived columns"),
            "{error}"
        );
        let health = client
            .health(pipestream_search::pb::HealthRequest {})
            .await
            .unwrap()
            .into_inner();
        assert_eq!(health.document_slots, 0);
        server.abort();
        let _ = server.await;
    }
}
